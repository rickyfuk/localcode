...
function todayWeather() {
	...
	$.ajax({
		...
		// add id and class for todayDiv1
		$(todayDiv1).addClass('row col-12');
		$(todayDiv1).attr('id', 'resultTodayrow');

		if (response.cod != 200) {
			// Create a h2 tag to hold the invalid input
			var todayInvalidInput = $('<h2>');
			// add class for todayh2
			$(todayInvalidInput).addClass('resultrowMain');
			// invalid message
			$(todayInvalidInput).text('Invalid Input or not found');
			// append todayH2 to resultToday Div
			$('#resultTodayrow').append(todayInvalidInput);

			// create a div for the temp result
			var todayInvalidInputDiv = $('<div>');
			// add class for temp result
			$(todayInvalidInputDiv).addClass('resultDetail row col-12');
			// add the temp to the todaytemp Div
			$(todayInvalidInputDiv).html('<p> Your input is invalid </p>');
			$(todayInvalidInputDiv).html(
				'<p> Suggest for the following action: </p>'
			);
			$(todayInvalidInputDiv).html(
				'<p> 1. Check your input to make sure the spelling of the city is correct </p>'
			);
			$(todayInvalidInputDiv).html(
				'<p> 2. Enter the country code after the city name(e.g. London,GB) </p>'
			);
			$(todayInvalidInputDiv).html(
				'<p> 2. (For US cities) ENter the state and country code after the city name(e.g. San Francisco,CA,US) </p>'
			);
			// append Temp Div to resultToday Div
			$('#resultTodayrow').append(todayInvalidInputDiv);
		} else {
